...
const struct {
  ...
  {static_cast<ExifTag>(MNOTE_PENTAX_TAG_MODE),
    { {0, N_("Auto")},
      {1, N_("Night scene")},
      {2, N_("Manual")},
      {4, N_("Multi-exposure")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_QUALITY),
    { {0, N_("Good")},
      {1, N_("Better")},
      {2, N_("Best")},{0,NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_FOCUS),
    { {2, N_("Custom")},
      {3, N_("Auto")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_FLASH),
    { {1, N_("Auto")},
      {2, N_("Flash on")},
      {4, N_("Flash off")},
      {6, N_("Red-eye reduction")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_WHITE_BALANCE),
    { {0, N_("Auto")},
      {1, N_("Daylight")},
      {2, N_("Shade")},
      {3, N_("Tungsten")},
      ...
      {5, N_("Manual")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_SHARPNESS),
    { {0, N_("Normal")},
      {1, N_("Soft")},
      {2, N_("Hard")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_CONTRAST),
    { {0, N_("Normal")},
      {1, N_("Low")},
      {2, N_("High")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_SATURATION),
    { {0, N_("Normal")},
      {1, N_("Low")},
      {2, N_("High")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_ISO_SPEED),
    { {10,  N_("100")},
      {16,  N_("200")},
      {100, N_("100")},
      {200, N_("200")},
      { 0,  NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX_TAG_COLOR),
    { {1, N_("Full")},
      {2, N_("Black & white")},
      {3, N_("Sepia")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_MODE),
    { {0, N_("Auto")},
      {1, N_("Night scene")},
      {2, N_("Manual")},
      {4, N_("Multi-exposure")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_QUALITY),
    { {0, N_("Good")},
      {1, N_("Better")},
      {2, N_("Best")},
      {3, N_("TIFF")},
      {4, N_("RAW")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_IMAGE_SIZE),
    { {0, "640x480"},
      {1, N_("Full")},
      {2, "1024x768"},
      {3, "1280x960"},
      ...
      {36, "3008x2008"},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_PICTURE_MODE),
    { {0, N_("Program")},
      {2, N_("Program AE")},
      {3, N_("Manual")},
      {5, N_("Portrait")},
      ...
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_FLASH_MODE),
    { {0x0000, N_("Auto, did not fire")},
      ...
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_FOCUS_MODE),
    { {0, N_("Normal")},
      {1, N_("Macro")},
      {2, N_("Infinity")},
      {3, N_("Manual")},
      ...
      {17, N_("AF-C")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_AFPOINT_SELECTED),
    { {1, N_("Upper-left")},
      {2, N_("Top")},
      {3, N_("Upper-right")},
      {4, N_("Left")},
      ...
      {0xffff, N_("Auto")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_AUTO_AFPOINT),
    { {0, N_("Multiple")},
      {1, N_("Top-left")},
      {2, N_("Top-center")},
      {3, N_("Top-right")},
      ...
      {0xffff, N_("None")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_WHITE_BALANCE),
    { {0, N_("Auto")},
      {1, N_("Daylight")},
      {2, N_("Shade")},
      {3, N_("Fluorescent")},
      ...
      {0, NULL}}},
  {static_cast<ExifTag>(MNOTE_CASIO2_TAG_BESTSHOT_MODE), 
    { {0, N_("Off")},
      {1, N_("On")},
      {0, NULL}}},
#endif
  {static_cast<ExifTag>(MNOTE_TAG_NULL0), {{0, NULL}}}
};
...
const struct {
  ...
  { static_cast<ExifTag>(MNOTE_PENTAX2_TAG_PICTURE_MODE),
    { {0,   0, N_("Auto")},
      ...
      {0,   0, NULL}}},
#endif
  {static_cast<ExifTag>(MNOTE_TAG_NULL0), {{0, 0, NULL}}}
};

char *MnotePentaxEntry::mnote_pentax_entry_get_value (char *val, unsigned int maxlen)
{
	ExifLong vl;
	ExifShort vs, vs2;
	int i = 0, j = 0;
	...
	switch (tag) {
	  ...
	  case MNOTE_CASIO2_TAG_BESTSHOT_MODE:
		CF (format, EXIF_FORMAT_SHORT, val, maxlen);
		CC2 (components, 1, 2, val, maxlen);
		if (components == 1) {
			...
			CC2 (components, 1, 2, val, maxlen);
			vs = exif_get_short (data, order);
			vs2 = exif_get_short (data+2, order) << 16;

			/* search the tag */
			for (i = 0; (items2[i].tag && items2[i].tag != tag); i++);
			if (!items2[i].tag) {
				snprintf (val, maxlen,
	...
	case MNOTE_PENTAX_TAG_ZOOM:
		CF (format, EXIF_FORMAT_LONG, val, maxlen);
		CC (components, 1, val, maxlen);
		vl = exif_get_long (data, order);
		snprintf (val, maxlen, "%li", (long int) vl);
		break;
	case MNOTE_PENTAX_TAG_PRINTIM:
		CF (format, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC (components, 124, val, maxlen);
	...
	case MNOTE_PENTAX2_TAG_DATE:
		...
		vs = exif_get_short (data, EXIF_BYTE_ORDER_MOTOROLA);
		snprintf (val, maxlen, "%i:%02i:%02i", vs, data[2], data[3]);
		break;
	case MNOTE_PENTAX2_TAG_TIME:
		CF (format, EXIF_FORMAT_UNDEFINED, val, maxlen);
		CC2 (components, 3, 4, val, maxlen);
		snprintf (val, maxlen, "%02i:%02i:%02i", data[0], data[1], data[2]);
		break;
	default:
		switch (format) {
		...
		case EXIF_FORMAT_SHORT:
		  {
			const unsigned char *data1 = data;
		  	size_t k, len = strlen(val);
		  	for(k=0; k<components; k++) {
				vs = exif_get_short (data1, order);
