...
const struct {
  ...
  { static_cast<ExifTag>(MNOTE_NIKON_TAG_LENSTYPE), EXIF_FORMAT_BYTE,
    { {0, N_("AF non D lens")},
      {1, N_("Manual")},
      ...
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON_TAG_FLASHUSED), EXIF_FORMAT_BYTE,
    { {0, N_("Flash did not fire")},
      {4, N_("Flash unit unknown")},
      {7, N_("Flash is external")},
      {9, N_("Flash is on camera")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_QUALITY), EXIF_FORMAT_SHORT,
    { {1, N_("VGA basic")},
      {2, N_("VGA normal")},
      {3, N_("VGA fine")},
      {4, N_("SXGA basic")},
      ...
      {12, N_("2 Mpixel fine")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_COLORMODE), EXIF_FORMAT_SHORT,
    { {1, N_("Color")},
      {2, N_("Monochrome")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_IMAGEADJUSTMENT), EXIF_FORMAT_SHORT,
    { {0, N_("Normal")},
      {1, N_("Bright+")},
      {2, N_("Bright-")},
      {3, N_("Contrast+")},
      {4, N_("Contrast-")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_CCDSENSITIVITY), EXIF_FORMAT_SHORT,
    { {0, N_("ISO 80")},
      {2, N_("ISO 160")},
      {4, N_("ISO 320")},
      {5, N_("ISO 100")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_WHITEBALANCE), EXIF_FORMAT_SHORT,
    { {0, N_("Auto")},
      {1, N_("Preset")},
      {2, N_("Daylight")},
      {3, N_("Incandescence")},
      ...
      {6, N_("SpeedLight")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_NIKON1_TAG_CONVERTER), EXIF_FORMAT_SHORT,
    { {0, N_("No fisheye")},
      {1, N_("Fisheye on")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_QUALITY), EXIF_FORMAT_SHORT,
    { {1, N_("Normal, SQ")},
      {2, N_("Normal, HQ")},
      {3, N_("Normal, SHQ")},
      {4, N_("Normal, RAW")},
      ...
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_MACRO), EXIF_FORMAT_SHORT,
    { {0, N_("No")},
      {1, N_("Yes")},
      {2, N_("Super macro")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_BWMODE), EXIF_FORMAT_SHORT,
    { {0, N_("No")},
      {1, N_("Yes")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_ONETOUCHWB), EXIF_FORMAT_SHORT,
    { {0, N_("Off")},
      {1, N_("On")},
      {2, N_("On (Preset)")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_FLASHMODE), EXIF_FORMAT_SHORT,
    { {0, N_("Auto")},
      {1, N_("Red-eye reduction")},
      {2, N_("Fill")},
      {3, N_("Off")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_FLASHDEVICE), EXIF_FORMAT_SHORT,
    { {0, N_("None")},
      {1, N_("Internal")},
      {4, N_("External")},
      {5, N_("Internal + external")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_FOCUSRANGE), EXIF_FORMAT_SHORT,
    { {0, N_("Normal")},
      {1, N_("Macro")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_MANFOCUS), EXIF_FORMAT_SHORT,
    { {0, N_("Auto")},
      {1, N_("Manual")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_SHARPNESS), EXIF_FORMAT_SHORT,
    { {0, N_("Normal")},
      {1, N_("Hard")},
      {2, N_("Soft")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_EXTERNALFLASHBOUNCE), EXIF_FORMAT_SHORT,
    { {0, N_("No")},
      {1, N_("Yes")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_CONTRAST), EXIF_FORMAT_SHORT,
    { {0, N_("Hard")},
      {1, N_("Normal")},
      {2, N_("Soft")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_PREVIEWIMAGEVALID), EXIF_FORMAT_LONG,
    { {0, N_("No")},
      {1, N_("Yes")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_OLYMPUS_TAG_CCDSCANMODE), EXIF_FORMAT_SHORT,
    { {0, N_("Interlaced")},
      {1, N_("Progressive")},
      {0, NULL}}},

  { static_cast<ExifTag>(MNOTE_SANYO_TAG_SEQUENTIALSHOT), EXIF_FORMAT_SHORT,
    { {0, N_("None")},
      {1, N_("Standard")},
      {2, N_("Best")},
      {3, N_("Adjust exposure")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_FOCUSMODE), EXIF_FORMAT_SHORT,
    { {1, N_("Spot focus")},
      {2, N_("Normal focus")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_RECORDSHUTTERRELEASE), EXIF_FORMAT_SHORT,
    { {0, N_("Record while down")},
      {1, N_("Press start, press stop")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_RESAVED), EXIF_FORMAT_SHORT,
    { {0, N_("No")},
      {1, N_("Yes")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_CCDSENSITIVITY), EXIF_FORMAT_SHORT,
    { {0, N_("Auto")},
      {1, N_("ISO 50")},
      {3, N_("ISO 100")},
      {4, N_("ISO 200")},
      {5, N_("ISO 400")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_SCENESELECT), EXIF_FORMAT_SHORT,
    { {0, N_("Off")},
      {1, N_("Sport")},
      {2, N_("TV")},
      {3, N_("Night")},
      ...
      {6, N_("Lamp")},
      {0, NULL}}},
  { static_cast<ExifTag>(MNOTE_SANYO_TAG_SEQUENCESHOTINTERVAL), EXIF_FORMAT_SHORT,
    { {0, N_("5 frames/sec")},
      {1, N_("10 frames/sec")},
      {2, N_("15 frames/sec")},
      {3, N_("20 frames/sec")},
      {0, NULL}}},
#endif
  {static_cast<ExifTag>(MNOTE_TAG_NULL), EXIF_FORMAT_NULL, { { 0, NULL } } }
};

char *MnoteOlympusEntry::mnote_olympus_entry_get_value (char *v, unsigned int maxlen)
{
	char         buf[30];
	ExifLong     vl;
	ExifShort    vs = 0;
	...
	case MNOTE_NIKON_TAG_FIRMWARE:
		CF (format,  EXIF_FORMAT_UNDEFINED, v, maxlen);
		CC (components, 4, v, maxlen);
		vl = exif_get_long (data, order);
		if ((vl & 0xF0F0F0F0) == 0x30303030) {
			memcpy (v, data, MIN (maxlen, 4));
		} else {
			snprintf (v, maxlen, "%04lx", (long unsigned int) vl);
		}
		break;
	case MNOTE_NIKON_TAG_ISO:
                CF (format, EXIF_FORMAT_SHORT, v, maxlen);
                CC (components, 2, v, maxlen);
                /*vs = exif_get_short (data, order);*/
                vs = exif_get_short (data + 2, order);
	...
	case MNOTE_EPSON_TAG_SOFTWARE:
		CF (format, EXIF_FORMAT_ASCII, v, maxlen);
		memcpy(v, data, MIN (maxlen, size));
		break;
	case MNOTE_NIKON_TAG_TOTALPICTURES:
	case MNOTE_EPSON_TAG_IMAGE_WIDTH:
	case MNOTE_EPSON_TAG_IMAGE_HEIGHT:
		CF (format, EXIF_FORMAT_LONG, v, maxlen);
		CC (components, 1, v, maxlen);
		vl =  exif_get_long (data, order);
	...
	case MNOTE_OLYMPUS_TAG_LENSTEMPERATURE:
		CF (format, EXIF_FORMAT_SSHORT, v, maxlen);
		CC (components, 1, v, maxlen);
		vs = exif_get_short (data, order);
		snprintf (v, maxlen, "%hd", vs);
		break;
	case MNOTE_NIKON_TAG_WHITEBALANCERB:
		CF (format, EXIF_FORMAT_RATIONAL, v, maxlen);
		CC (components, 4, v, maxlen);
		vr = exif_get_rational (data, order);
		r = R2D(vr);
		vr = exif_get_rational (data+8, order);
		b = R2D(vr);
	...
	case MNOTE_NIKON_TAG_SENSORPIXELSIZE:
		...
		CC (components, 2, v, maxlen);
		vr = exif_get_rational (data, order);
		vr2 = exif_get_rational (data+8, order);
		r = R2D(vr);
		b = R2D(vr2);
		snprintf (v, maxlen, "%2.2f x %2.2f um", r, b);
		break;
	case MNOTE_NIKON_TAG_BRACKETING:
		CF2 (format, EXIF_FORMAT_BYTE, EXIF_FORMAT_SHORT, v, maxlen);
		CC (components, 1, v, maxlen);
	...
	case MNOTE_OLYMPUS_TAG_FLASHDEVICE:
		...
		vs = exif_get_short(data, order);
		/* search for the tag */
		for (i = 0; (items[i].tag && items[i].tag != tag); i++)
			;
		if (!items[i].tag) {
		  	snprintf (v, maxlen, _("Internal error (unknown value %hi)"), vs);
		  	break;
		}
		CF (format, items[i].fmt, v, maxlen);
		/* find the value */
	...
	case MNOTE_NIKON_TAG_LENS:
		...
		CC (components, 4, v, maxlen);
		{
			...
			vr = exif_get_rational (data+8, order);
			b = R2L(vr);
			vr = exif_get_rational (data+16, order);
			c = R2D(vr);
			vr = exif_get_rational (data+24, order);
			d = R2D(vr);
	...
	case MNOTE_OLYMPUS_TAG_MODE:
		...
		vl = exif_get_long (data + 4, order);
		snprintf (buf, sizeof (buf), "/%li/", (long int) vl);
		strncat (v, buf, maxlen - strlen (v));
		vl = exif_get_long (data + 8, order);
		switch (vl) {
		case 1:
	...
	case MNOTE_OLYMPUS_TAG_LENSDISTORTION:
		if (format == EXIF_FORMAT_SHORT) {
			...
			CC (components, 6, v, maxlen);
			CF (format, EXIF_FORMAT_SSHORT, v, maxlen);
			for (i=0; i < (int)components; ++i) {
				vss = exif_get_sshort (data+2*i, order);
				sprintf (buf, "%hd ", vss);
				strncat (v, buf, maxlen - strlen (v));
			}
		}
	...
	case MNOTE_OLYMPUS_TAG_COLORCONTROL:
		...
		CC (components, 6, v, maxlen);
		for (i=0; i < (int)components; ++i) {
			vs = exif_get_short (data+2*i, order);
			sprintf (buf, "%hu ", vs);
			strncat (v, buf, maxlen - strlen (v));
		}
		break;
	...
	case MNOTE_OLYMPUS_TAG_INFO:
		CF (format, EXIF_FORMAT_ASCII, v, maxlen);
		CC2 (components, 52, 60, v, maxlen);
		strncpy (v, (char *)data, MIN (maxlen, size));
		break;
	case MNOTE_OLYMPUS_TAG_ID:
		CF (format, EXIF_FORMAT_UNDEFINED, v, maxlen);
		CC (components, 32, v, maxlen);
		strncpy (v, (char *)data, MIN (maxlen, size));
		break;
	case MNOTE_OLYMPUS_TAG_UNKNOWN_4:
		CF (format, EXIF_FORMAT_LONG, v, maxlen);
		CC (components, 30, v, maxlen);
		for (i=0; i < (int)components; ++i) {
			vl = exif_get_long (data+4*i, order);
	...
	case MNOTE_OLYMPUS_TAG_WBALANCE:
		...
		case 2:
			{
				ExifShort v2 = exif_get_short (data + 2, order);
	...
	case MNOTE_OLYMPUS_TAG_COLORMATRIX:
		CF (format, EXIF_FORMAT_SHORT, v, maxlen);
		if (tag == MNOTE_OLYMPUS_TAG_COLORMATRIX)
			CC (components, 9, v, maxlen);
		for (i=0; i < (int)components; ++i) {
			vs = exif_get_short (data+2*i, order);
			sprintf (buf, "%hu ", vs);
			strncat (v, buf, maxlen - strlen (v));
	...
	default:
		...
		case EXIF_FORMAT_SRATIONAL:
			CC (components, 1, v, maxlen);
			vsr = exif_get_srational (data, order);
			if (!vsr.denominator) {
				strncpy (v, _("Infinite"), maxlen);
			} else {
				r = R2D(vsr);
