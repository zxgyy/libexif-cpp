...
unsigned int MnoteCanonEntry::mnote_canon_entry_count_values ()
{
	unsigned int  val;
	...
	case MNOTE_CANON_TAG_COLOR_INFORMATION:
		if (format != EXIF_FORMAT_SHORT) return 0;

		val = exif_get_short (data, order);
		/* val is buffer size, i.e. # of values plus 1 */
		return MIN (size - 2, val) / 2;
	default:
		return 1;
	}
}
...
char *MnoteCanonEntry::mnote_canon_entry_get_value (unsigned int t, char *val, unsigned int maxlen)
{
	char buf[128];
	ExifLong vl;
	ExifShort vs, n;
	...
	case MNOTE_CANON_TAG_SETTINGS_1:
		...
		if (t >= n) return NULL;
		CC (components, n, val, maxlen);
		vs = exif_get_short (data + 2 + t * 2, order);
		switch (t) {
		case 1:
			if (!vs) {
	...
	case MNOTE_CANON_TAG_FOCAL_LENGTH:
		CF (format, EXIF_FORMAT_SHORT, val, maxlen);
		vs = exif_get_short (data + t * 2, order);
		switch (t) {
		case 1:
			snprintf (val, maxlen, "%u", vs);
			break;
	...
	case MNOTE_CANON_TAG_OWNER:
		CC (components, 32, val, maxlen);
		/* Fall through; ImageType can have many sizes */
	case MNOTE_CANON_TAG_IMAGE_TYPE:
		CF (format, EXIF_FORMAT_ASCII, val, maxlen);
		strncpy (val, (char *)data, MIN (size, maxlen));
		break;

	case MNOTE_CANON_TAG_FIRMWARE:
		CF (format, EXIF_FORMAT_ASCII, val, maxlen);
/*		CC2 (components, 24, 32, val, maxlen); Can also be 22 */
		strncpy (val, (char *)data, MIN (size, maxlen));
		break;
	...
	default:
#ifdef DEBUG
	  {
		...
		else if (format == EXIF_FORMAT_LONG)
		for(i=0;i<components;i++) {
			vl = exif_get_long (data, order);
			data+=4;
			printf ("Value%d=%d\n", i, vs);
		}
		else if (format == EXIF_FORMAT_ASCII)
		    strncpy (val, data, MIN (size, maxlen));
	  }
#endif
		break;
	}
